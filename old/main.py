...
collection = dbname.caltech_101_features_final
...
for image in image_collection.find():
    ...
    img = cv2.resize(numpy.array(img), (300, 100))

        # image is being read as JpegImageFile Class for input to torchvision transformation models
        # resnet_input = numpy.array(image["image"], dtype=numpy.uint8)
        # print(type(torch.Tensor(numpy.array(image["image"]))))
        ...
        if True:
            ...
            image_features["image_id"] = image["image_id"]
