...
image_features = {}
ress = {}
img_path = '/home/abhinavgorantla/hdd/ASU/Fall 23 - 24/CSE515 - Multimedia and Web Databases/caltech-101/101_ObjectCategories/Faces/image_0001.jpg'
...
if image_tensor.size()[1] == 3:
    ...
    for document in collection.find():
        ...
        d = (tq)/(mt+mq)
        if((str(d**(0.5))) in ress):
            ress[str(d**(0.5))].append(document["image_name"])
        else:
            ress[str(d**(0.5))] = [document["image_name"]]
        print(document["image_name"])
    
    # sorted(ress)
    keys = sorted(list(ress.keys()), reverse=True)
    for i in range(10):
        print(sorted(ress[keys[i]], reverse = True))
        print(keys[i])
    # print(ress[keys[1]])
    # print(keys[0])
    # print(keys[3])
