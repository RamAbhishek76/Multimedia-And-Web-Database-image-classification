###### Testing Script ######
# Functionality: Used this as a test script for querying images using color moment feature
import torch
import cv2
import math
import os
...
def query_color_moment(image_id):
    im_id = str(image_id)
    ...
    query_image_color_moment = numpy.array(
        query_image_features['color_moment'])
    if len(img) == 3:
        for document in collection.find({}):
            print(document["image_id"])
            test = numpy.array(document["color_moment"]).flatten()
            if len(test) > 0:
                d = distance.cosine(test, query_image_color_moment.flatten())
                if (d in results):
                    results[d].append(
        ...
        fig.suptitle(
            'Color moment query top 10 outputs for input image ID ' + str(image_id), fontsize=16)
