...
def output_plotter(feature_name, input_image, feature_vals, feature_val_keys, k):
    ...
    fig.suptitle(feature_name + ' query top 10 outputs for input image ID ' + str(input_image["image_id"]), fontsize=16)

    plt.imshow((numpy.squeeze(torch.tensor(numpy.array(input_image["image"])).permute(1 , 2 , 0))))
    plt.title("Query Image ID: " + str(input_image["image_id"]))
    
    print("Results for " + feature_name + ":")
    for i in range(k):
        print(feature_vals[feature_val_keys[i]])
        # print(str(feature_vals[feature_val_keys[i]]) + " " + str(feature_vals[feature_val_keys[i]]))
        image = collection.find_one({"image_id": feature_vals[feature_val_keys[i]]})
